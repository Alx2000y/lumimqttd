...
void *blescan_thread(void *args)
{
	...
	if (dd < 0)
	{
		_syslog(LOG_ERR, "Could not open device");
		return;
	}
	...
	while (1)
	{
		...
		topic = malloc(strlen(config.topic) + 21);
		message = malloc(strlen(name) + 20);
